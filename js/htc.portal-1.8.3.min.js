/*定义网站全局变量 陈明森 2012.9.5*/
var UploadUrl = "http://upload3.haitaocheng.com/";
var ImgServerUrl = "http://img.haitaocheng.com/";
var PayUrl = "http://pay.haitaocheng.com/";
var PassportUrl = "http://passport.haitaocheng.com/";
var PortalUrl = "http://www.haitaocheng.com/";
var MemberUrl = "http://member.haitaocheng.com/";
var CartUrl = "http://cart.haitaocheng.com/";
var asyncConfig = true;//更改ajax为同步还是异步
...
var __FunAssemblyName = "HTC.Business.dll";
var __ParamAssemblyName = "HTC.Entity.dll";
...
$.fn.fillTableTemplate = function (g) {
    return this.each(function () {
        ...
        if (url == "http:/") url = document.location;
...
$.ajaxRequest = function (e) {
    ...
    var i = {
        ...
        success: function (b) {
            ...
            if (c.Complete.toLowerCase() == 'succeed') {
                ...
                if (c.Result == undefined) mValue = null;
                ...
                //                    return
                //                }
                if ((mValue == null || mValue == "") && c.ResultType.toLowerCase() == "value") {
                    if (requestConfigs.onComplete) {
                        requestConfigs.onComplete(mValue, null)
                    }
                    return;
            ...
            if (f.target != null) $("#" + tableID + "-Loading").html("")
        }
    };
    if (f.initData != null) {
    ...
    function serializeRequest(a, b) {
        if (f.page.AllowPaging) {
            ...
            b.RecordCount = a.page.RecordCount;
            var c = $("#" + $(a.target).attr("id") + "_PageIndex");
            if (a.page.PageIndex < 1) {
                b.PageIndex = $(c).length == 0 ? 1 : $(c).val()
            } else {
                b.PageIndex = a.page.PageIndex
            }
        }
        var d = {
...
function Pageing(b, thisID) {
    if (b.AllowPaging == true && b.RecordCount > b.PageSize) {
        if ($("#" + thisID + "_PageBarTemp").html() == null) {
            $("#" + thisID + "_PageBar").append("<div class=\"page\" id=" + thisID + "_PageBarTemp> </div>")
        }
        $("#" + thisID + "_PageBarTemp").pageBar(b)
    } else $("#" + thisID + "_PageBar").html("")
}


Function.prototype.bind = function (o) { var _f = this; return function () { return _f.apply(o, arguments); }; }; $.fn.outer = function () { return $('<div></div>').append(this.eq(0).clone()).html(); }; jQuery.extend({
    evalJSON: function (strJson) { return eval("(" + strJson + ")"); }, toJSON: function (object) {
        var type = typeof object; if ('object' == type) {
            if (Array == object.constructor)
                type = 'array'; else if (RegExp == object.constructor)
                type = 'regexp'; else
                type = 'object';
        }
        switch (type) {
            case 'undefined': case 'unknown': return; break; case 'function': case 'boolean': case 'regexp': return object.toString(); break; case 'number': return isFinite(object) ? object.toString() : 'null'; break; case 'string': return '"' + object.replace(/(\\|\")/g, "\\$1").replace(/\n|\r|\t/g, function () { var a = arguments[0]; return (a == '\n') ? '\\n' : (a == '\r') ? '\\r' : (a == '\t') ? '\\t' : "" }) + '"'; break; case 'object': if (object === null) return 'null'; var results = []; for (var property in object) { if (object[property] != null) { var value = jQuery.toJSON(object[property]); if (value !== undefined) { results.push(jQuery.toJSON(property) + ':' + value); } } else { results.push(property + ':null'); } }
                return '{' + results.join(',') + '}'; break; case 'array': var results = []; for (var i = 0; i < object.length; i++) { var value = jQuery.toJSON(object[i]); if (value !== undefined) results.push(value); }
    ...
    return this.each(function () {
        var t = this.type, tag = this.tagName.toLowerCase(), eEl = this; alert(t); switch (t) {
            case 'hidden': case 'text': case 'file': case 'password': case 'textarea': this.value = sValue; break; case 'checkbox': var oReg = new RegExp('(^|,) + eEl.value + (,|$)', 'g'); eEl.checked = oReg.test(sValue); break; case 'radio': if (eEl.value == sValue)
                eEl.checked = true; break; case 'select-one': for (var j = 0; j < eEl.options.length; j++) { if (eEl.options[j].value == sValue) { eEl.options[j].selected = true; break; } }
                break;
        }
    });
    ...
    if (setValue == undefined) {
        if (type == 'checkbox' || type == 'radio')
            return $('input[name=' + typeName + ']:checked').val(); else
        ...
        $(":" + type + "[name=" + typeName + "]").each(function (i) {
            if (setValue == this.value) {
                if (type == 'checkbox' || type == 'radio')
                    $("#" + this.id).attr("checked", "true"); else
                    $("#" + this.id).attr("select", "true");
            }
        });
    }
}; Number.prototype.numFormat = function () { var temp = this.toFixed(2); return temp.numFormat(); }
String.prototype.numFormat = function () {
    var value = this; var indexOf = value.indexOf("."); if (indexOf >= 0) {
        var temp = value.substring(0, indexOf) + value.substring(indexOf, indexOf + 3); if (value.length == indexOf + 2)
            return temp + "0"; else if (value.length == indexOf + 1)
            return temp + "00"; else
            return temp;
    }
    else { return value + ".00"; }
}
Number.prototype.currencyFormat = function () { var temp = this.toFixed(2); temp = temp.numFormat(); temp = "&yen;" + temp; return temp; }
String.prototype.replaceAll = function (s1, s2) { return this.replace(new RegExp(s1, "gm"), s2); }
function JsAlert(msg) { $("#spnError").html(msg); }
function JsAlertAgo(msg) { alert(msg); history.go(-1); }
function JsAlertHref(msg, href) { alert(msg); window.location.href = href; }
...
function FloatDiv(divID) {
    var _over = { overlayOpacity: 0.9, overlayColor: '#333333' }
    $('body').append('<div id="d_overlay" style="overflow:hidden;"></div>'); var lefts = (974 - $(window).width()) / 2; $("#d_overlay").css({ position: 'absolute', zIndex: 9998, top: 0, left: lefts + 'px', height: $(document).height(), width: $(document).width(), background: _over.overlayColor, opacity: _over.overlayOpacity, textAlign: 'center' }); $('#' + divID).css({ position: 'absolute', zIndex: 9999, left: (974 - $('#' + divID).width()) / 2, top: $(document).scrollTop() + ($(window).height() - $('#' + divID).height()) / 2, display: "block" }); $('body').attr('scroll', 'no'); document.body.style.overflow = "hidden"; $('body').css('overflow-y', 'hidden'); $('#winclose').click(function () { CloseFloatDiv(divID); });
}
function CloseFloatDiv(divID) { $("#d_overlay").html(''); $("#d_overlay").remove(); $('body').attr('scroll', ''); document.body.style.overflow = 'auto'; $('body').css('overflow-y', ''); $('#' + divID).css('display', 'none'); }
function FloatFrame(pageLink, width, height) {
    var _over = { overlayOpacity: 1.0, overlayColor: '#ccc' }
    $('body').append('<div id="f_overlay" style="overflow:hidden"><div id="f_win"  style="z-index:9999;width:' + width + 'px;height:' + height + 'px;"><iframe width="' + width + 'px"  height=' + height + 'px" frameborder="0" src="' + pageLink + '" scrolling="no" ></iframe></div></div>'); var divID = "f_win"; var lefts = $(window).width() / 2 - $('#' + divID).width() / 2; $('#' + divID).css({ position: 'absolute', left: $(window).width() / 2 - $('#' + divID).width() / 2, top: $(document).scrollTop() + ($(window).height() - $('#' + divID).height()) / 2, display: "block" }); $("#f_overlay").css({ position: 'absolute', zIndex: 9998, top: 0, left: '0px', height: $(document).height(), width: $(document).width(), background: _over.overlayColor, opacity: _over.overlayOpacity, textAlign: 'center' }); $(window).bind("resize", function () { $('#' + divID).css({ left: $(window).width() / 2 - $('#' + divID).width() / 2, top: $(document).scrollTop() + ($(window).height() - $('#' + divID).height()) / 2 }); $("#f_overlay").css({ height: $(document).height(), width: $(window).width() }); });
}
function CloseFloatFrame() { parent.document.location.reload(); $('#f_overlay', parent.document).remove(); }
function FloatFrameAuto(pageLink, width, height) {
    if (height == 0) {
        height = $(window).height();
        var browser = getBrowserInfo();
        var verinfo = (browser + "").replace(/[^0-9.]/ig, "");
        if (browser == undefined || browser[0].indexOf("msie") > -1) {
    ...
    if (width == 0)
        width = $(window).width(); var _over = { overlayOpacity: .95, overlayColor: '#ccc' }
    $('body').append('<div id="f_overlay" style="overflow:auto"><div id="f_win"  style="z-index:9999;width:' + width + 'px;height:' + height + 'px;"><iframe width="' + width + 'px"  height=' + height + 'px" frameborder="0" src="' + pageLink + '" scrolling="no" ></iframe></div></div>'); var divID = "f_win"; var lefts = 974 / 2 - $('#' + divID).width() / 2; $('#' + divID).css({ position: 'absolute', left: $(window).width() / 2 - $('#' + divID).width() / 2, top: $(document).scrollTop() + ($(window).height() - $('#' + divID).height()) / 2, display: "block" }); $("#f_overlay").css({ position: 'absolute', zIndex: 9998, top: 0, left: 0, height: $(document).height(), width: $(document).width(), background: _over.overlayColor, opacity: _over.overlayOpacity, textAlign: 'center' });

}
var charset = 'utf-8'; function mb_strlen(str) {
...
function mb_cutstr(str, maxlen, dot) {
    var len = 0; var ret = ''; var dot = !dot ? '...' : ''; maxlen = maxlen - dot.length; for (var i = 0; i < str.length; i++) {
        len += str.charCodeAt(i) < 0 || str.charCodeAt(i) > 255 ? (charset == 'utf-8' ? 3 : 2) : 1; if (len > maxlen) { ret += dot; break; }
        ret += str.substr(i, 1);
    }
    return ret;
}
...
function textSelect(obj) { var length = obj.value.length; if (length) { if (obj.createTextRange) { var range = obj.createTextRange(); range.moveStart("character", -length); range.moveEnd("character", -length); range.moveStart("character", 0); range.moveEnd("character", length); range.select(); } else { obj.setSelectionRange(0, length); obj.focus(); } } }
function ShowTools(msg, top) {
    var addtop = 0; if (top != undefined) { addtop = top; }
    $("#divSysTools").remove(); var html = "<div id=\"divSysTools\" style=\"position:absolute\">"; html += "<table cellpadding=0 cellspacing=0 border=0><tr><td style='background:url(" + ImgServerUrl + "/images/common/prompt_window_bg.png) 0 0 no-repeat; width:59px;height:36px;'></td><td id='spSysTools' style='color:#fff; text-align:center;background:url(" + ImgServerUrl + "/images/common/prompt_window_bg.png) 0 -40px repeat-x; line-height:36px;'>" + msg + "</td><td style='background:url(" + ImgServerUrl + "/images/common/prompt_window_bg.png) 0 -81px no-repeat; width:59px;height:36px;'></td></tr></table>"; html += "</div>"; $('body').append(html); $('#divSysTools').css({ zIndex: 9999, left: ($(window).width() - $('#divSysTools').width()) / 2, top: $(document).scrollTop() + ($(window).height() - $('#divSysTools').height()) / 2 + addtop, display: "block" }); setTimeout(function () { $("#divSysTools").fadeOut(1000); }, 3000);
}
//获取浏览器版本
...
function ShowMsg(msg, reload) {
    var addtop = 0; $("#divSysTools").remove(); var html = "<div id=\"divSysTools\" style=\"position:absolute\">"; html += "<table cellpadding=0 cellspacing=0 border=0><tr><td style='background:url(" + ImgServerUrl + "/images/common/prompt_window_bg.png) 0 0 no-repeat; width:59px;height:36px;'></td><td id='spSysTools' style='color:#fff; text-align:center;background:url(" + ImgServerUrl + "/images/common/prompt_window_bg.png) 0 -40px repeat-x; line-height:36px;'>" + msg + "</td><td style='background:url(" + ImgServerUrl + "/images/common/prompt_window_bg.png) 0 -81px no-repeat; width:59px;height:36px;'></td></tr></table>"; html += "</div>"; $('body').append(html); $('#divSysTools').css({ zIndex: 9999, left: (974 - $('#divSysTools').width()) / 2, top: $(document).scrollTop() + ($(window).height() - $('#divSysTools').height()) / 2 + addtop, display: "block" }); setTimeout(function () { $("#divSysTools").fadeOut(1000); if (reload != undefined) { window.document.location.reload(); } }, 3000);
}